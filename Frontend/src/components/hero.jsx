...
const HeroSection = () => {
  const [editorContent, setEditorContent] = useState("print(\"Welcome to Golmaal\");");
  const [outputContent, setOutputContent] = useState("Welcome to Golmaal");
  ...
  useEffect(() => {
    // Reset hasRunSuccessfully flag on page load
    localStorage.removeItem('hasRunSuccessfully');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-gray-900/90 backdrop-blur-md rounded-2xl shadow-2xl border border-gray-700/50 p-8 flex flex-col items-center space-y-6">
        ...
        <button
          ...
          className="hover:cursor-pointer px-8 py-3 rounded-lg text-white font-medium bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-lg hover:shadow-green-500/25 transition-all duration-300 transform hover:scale-105 active:scale-95"
        >
          OK
        </button>
  ...
  const RunButton = () => (
    <button 
      ...
      disabled={isRunning}
      className={`px-2.5 py-1.5 bg-transparent backdrop-blur-md hover:bg-gray-800/30 transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-green-500/25 group flex items-center space-x-1.5 border border-[#1e1e1e] hover:cursor-pointer hover:border-green-500/30 rounded-lg ${isRunning ? 'opacity-50 cursor-not-allowed' : ''}`}
      title="Run Code"
    >
      <svg 
  ...
  const ClearButton = () => (
    <button 
      onClick={handleClear}
      className="px-2.5 hover:cursor-pointer py-1.5 bg-transparent backdrop-blur-md hover:bg-gray-800/30 rounded-lg transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-red-500/25 group flex items-center space-x-1.5 border border-[#1e1e1e] hover:border-red-500/30"
      title="Clear Output"
    >
      <svg 
